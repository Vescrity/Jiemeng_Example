...
local gemini_key = gemini_key or ''
local ex = require("jm_generic_extend")
local exs = ex.string
local table2str = ex.table2str
require("bot_api")
...
%s]=],
    ...
    if not status then
        ...
        elseif (response.promptFeedback ~= nil) then
            err = '似乎被捂嘴了…… ' .. response.promptFeedback.blockReason
        elseif (response.candidates[1] ~= nil) then
...
function Chat:chat_all(message, model)
    ...
    local md2png = require("pngutils").md2png
